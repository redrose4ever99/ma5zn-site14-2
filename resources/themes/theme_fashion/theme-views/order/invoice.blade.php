    ...
    <table class="content-position mb-30">
        <tr>
            ...
            <th class="p-0 text-right">
                <img loading="lazy" height="40" src="{{asset("storage/app/public/company/".$company_web_logo)}}" alt="{{ $company_name }}">
            </th>
        </tr>
    </table>
    ...
    <table class="bs-0 mb-30 px-10">
        <tr>
            <th class="content-position-y text-left">
                ...
                <h4 class="text-uppercase mb-1 fz-14">
                    {{ucwords('shop name')}}
                    : {{ $order->seller_is == 'admin' ? $company_name : (isset($order->seller->shop) ? $order->seller->shop->name : ucwords('not found')) }}
                </h4>
                ...
                <tr>
                    ...
                    <td>
                        <table>
                            <tr>
                                <td class="text-right">
                                    @if(!$order->is_guest && $order->billingAddress)
                                        <span class="h2" >{{ucwords('billing address')}} </span>
                                        <div class="h4 montserrat-normal-600">
                                            <p class="font-weight-normal mt-6px mb-0">{{$order->billingAddress ? $order->billingAddress['contact_person_name'] : ""}}</p>
                                            <p class="font-weight-normal mt-6px mb-0">{{$order->billingAddress ? $order->billingAddress['phone'] : ""}}</p>
                                            <p class="font-weight-normal mt-6px mb-0">{{$order->billingAddress ? $order->billingAddress['address'] : ""}}</p>
                                            <p class="font-weight-normal margin-top: 6px; margin-bottom:0px;">{{$order->billingAddress ? $order->billingAddress['city'] : ""}} {{$order->billingAddress ? $order->billingAddress['zip'] : ""}}</p>
                                        </div>
                                    @endif
                                </td>
                            </tr>
                ...
                @if ($order->delivery_type !=null)
                    ...
                    @if ($order->delivery_type == 'self_delivery')
                        <p class="fz-12 font-normal">
                            ...
                            <br>
                            <span class="font-weight-normal">
                                {{ucwords('deliveryman name')}} : {{$order->delivery_man['f_name'].' '.$order->delivery_man['l_name']}}
                            </span>
                            <br>
                            <span class="font-weight-normal">
                                {{ucwords('deliveryman phone')}} : {{$order->delivery_man['phone']}}
                            </span>
                        </p>
                    @else
                        <p>
                    ...
                    <tbody>
                        ...
                        @if($order->order_type == 'default_type')
                            <tr>
                                ...
                                <td class="p-1 text-right">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping - ($order->is_shipping_free ? $order->extra_discount : 0)))}}</td>
                            </tr>
                        @endif
                        <tr>
        ...
        <table>
            <tr>
                <th class="content-position-y bg-light py-4">
                    ...
                    <div>
                        {{ucwords('all copyright reserved © '.date('Y').' ').$company_name}}
                    </div>
                </th>
            </tr>
        </table>
